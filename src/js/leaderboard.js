...
const LeaderboardService = {
    ...
    /**
     * Check if a name is already taken by another player (case-insensitive).
     * Returns true if taken, false if available.
     */
    async isNameTaken(name) {
        if (!this.isReady() || !name) return false;
        try {
            // Check cached scores first (case-insensitive)
            const lowerName = name.toLowerCase();
            const takenInCache = this.cachedScores.some(
                s => s.name && s.name.toLowerCase() === lowerName && s.playerUID !== this.playerUID
            );
            if (takenInCache) return true;

            // Also check Firestore for exact match (covers entries not in top results)
